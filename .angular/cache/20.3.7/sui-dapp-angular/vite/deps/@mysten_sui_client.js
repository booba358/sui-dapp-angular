...
function parseTransactionBcs(bytes) {
  return __spreadProps(__spreadValues({}, TransactionDataBuilder.fromBytes(bytes).snapshot()), {
    bcs: bytes
  });
}
...
function parseTransaction(transaction) {
  ...
  return {
    ...
    objectTypes: Promise.resolve(objectTypes),
    transaction: __spreadProps(__spreadValues({}, data), {
      bcs: bytes
    }),
...
var SuiJsonRpcClient = class extends Experimental_BaseClient {
  ...
  async getOwnedObjects(input) {
    ...
    const filter = input.filter ? __spreadValues({}, input.filter) : void 0;
  ...
  async queryTransactionBlocks({
    ...
    if (filter && "MoveFunction" in filter && isValidNamedPackage(filter.MoveFunction.package)) {
      filter = __spreadProps(__spreadValues({}, filter), {
        MoveFunction: {
          package: (await this.core.mvr.resolvePackage({
