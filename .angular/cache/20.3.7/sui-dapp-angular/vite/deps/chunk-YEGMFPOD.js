...
var BcsStruct = class extends BcsType {
  constructor(_a) {
    var _b = _a, { name, fields } = _b, options = __objRest(_b, ["name", "fields"]);
...
var BcsEnum = class extends BcsType {
  constructor(_a) {
    var _b = _a, { fields } = _b, options = __objRest(_b, ["fields"]);
    const canonicalOrder = Object.entries(fields);
    super(__spreadProps(__spreadValues({
      read: (reader) => {
...
var BcsTuple = class extends BcsType {
  constructor(_a) {
    var _b = _a, { fields, name } = _b, options = __objRest(_b, ["fields", "name"]);
    super(__spreadProps(__spreadValues({
...
var bcs = {
   ...
   */
  u8(options) {
    return uIntBcsType(__spreadProps(__spreadValues({
      readMethod: "read8",
      writeMethod: "write8",
      size: 1,
      maxValue: 2 ** 8 - 1
   ...
   */
  u64(options) {
    return bigUIntBcsType(__spreadProps(__spreadValues({
      readMethod: "read64",
      writeMethod: "write64",
      size: 8,
      maxValue: 2n ** 64n - 1n
   ...
   */
  uleb128(options) {
    return dynamicSizeBcsType(__spreadProps(__spreadValues({
   ...
   */
  string(options) {
    return stringLikeBcsType(__spreadProps(__spreadValues({
  ...
  tuple(fields, options) {
    return new BcsTuple(__spreadValues({
      fields
    }, options));
  },
  /**
  ...
  struct(name, fields, options) {
    return new BcsStruct(__spreadValues({
      name,
      fields
    }, options));
  },
  ...
  enum(name, fields, options) {
    return new BcsEnum(__spreadValues({
      name,
      fields
    }, options));
  },
